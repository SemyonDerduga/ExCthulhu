...
def find(ctx, max_depth, start, amount, cached, cache_dir, exchange_list, current_node, current_amount):
    """
        Find winning transaction and print it.
    """
    asyncio.run(run_cmd(ctx, max_depth, exchange_list, start, amount, cache_dir, current_node, current_amount, cached))
